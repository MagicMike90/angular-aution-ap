...
export class _StaticProductService implements ProductService {
  ...
  getById(productId: number): Observable<Product> {
    return this.http
      ...
      .pipe(map(products => products.find(p => p.id === productId) as Product));
  }
  ...
  search(params: ProductSearchParams): Observable<Product[]> {
    return this.http.get<Product[]>('/data/products.json').pipe(map(products => this.filterProducts(products, params)));
  }
