...
const db$: Promise<DB> = readFile('./data/products.json', 'utf8').then(JSON.parse, console.error);

export async function getDistinctCategories(): Promise<string[]> {
  const distinctCategories = (await db$).map(p => p.categories).reduce((all, current) => all.concat(current), []);
